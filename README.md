...
`onSuccess` reports portal authentication/session readiness. Account-grant
changes arrive separately through `onEvent`, and they do not close the iframe:
...
await finatic.openPortal({
  ...
  onEvent: async (eventName, payload) => {
    if (eventName === 'account.grant.created') {
      await finatic.v1.listAccounts({ includeSyncStatus: true });
      // The host may close its portal surface here if that matches its UX.
    }
  },
});
```

Known account-grant lifecycle names are `account.grant.created`,
`account.grant.updated`, and `account.grant.revoked`. `portal-close` remains
user dismissal, and hosts must close explicitly if desired. Server-side
integrations should use the independent HTTPS `account.grant.*` webhooks rather
than iframe messages.
...
- Listen to auth, close, and account-grant portal lifecycle events for UX state handling.
