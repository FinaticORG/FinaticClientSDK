...
describe('appendStageToURL', () => {
  ...
  it('appends a single stage filter', () => {
    expect(appendStageToURL('https://portal.example.com/?token=abc', ['production'])).toBe(
      'https://portal.example.com/?token=abc&stage=production'
    );
  });

  it('uses stage=all when every stage is selected', () => {
    expect(
      appendStageToURL('https://portal.example.com/?token=abc', ['production', 'beta', 'alpha'])
